...
switch ($_GET['action'])
{
	case 'genProvider':
		...
		array_push($resultados, GeradorDeDados::callWebService('provider/add', $parameters));
		break;
...
class GeradorDeDados
{
	public static function callWebService($webservice, $parameters)
	{
		...
		$debugparam = isset($_GET['debug']) ? '?debug' : '';

		$curl = curl_init("http://tercom/api/site/$webservice$debugparam");
	...
	public static function genPessoa($comPontuacao = false, $sexo = null, $idade = null, $estado = null, $cidade = null)
	{
		...
		$pessoa['idade'] = $idade;
		$pessoa['email'] = sprintf('%s@tercom.com.br', substr($pessoa['email'], 0, strpos($pessoa['email'], '@')));
	...
	public static function genCEP($numero = true, $pontuacao = false, $estado = null, $cidade = null)
	{
		...
		$parameters = array('pontuacao' => $comPontuacao ? 'S' : 'N', 'cep_estado' => $estado, 'cep_cidade' => $cidade);
